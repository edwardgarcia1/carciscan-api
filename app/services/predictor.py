...
def get_carcinogenicity_model_data():
    ...
    if _carcinogenicity_model_data is None:
        try:
            model_path = "ml_models/carcinogenicity.pkl"
...
def get_route_model_data():
    ...
    if _route_model_data is None:
        try:
            model_path = "ml_models/route.pkl"
            with open(model_path, 'rb') as f:
...
if __name__ == '__main__':
    # Test Carcinogenicity Model
    carc_model_info = get_carcinogenicity_model_data()
    if "error" not in carc_model_info:
        num_features = len(carc_model_info['feature_names'])
        dummy_descriptors = np.random.rand(num_features).tolist()

        print("--- Testing Carcinogenicity Prediction ---")
        result = predict_carcinogenicity(dummy_descriptors)
        if result:
        ...
        print("--- Skipping Carcinogenicity Test (Model not found) ---")

    print("\n" + "=" * 40 + "\n")

    # Test Route Model
    route_model_info = get_route_model_data()
    if "error" not in route_model_info:
        # Note: The feature names should be the same, but we check just in case.
        num_features_route = len(route_model_info['feature_names'])
        dummy_descriptors_route = np.random.rand(num_features_route).tolist()

        print("--- Testing Route Prediction ---")
        result_route = predict_route(dummy_descriptors_route)
        if result_route:
            print("✅ Prediction successful!")
            print(f"Predicted Routes: {result_route['prediction']}")
            print("Confidence Scores:")
            for label, score in result_route['confidence_scores'].items():
                print(f"  - {label}: {score:.4f}")
        else:
            print("❌ Prediction failed.")
    else:
        print("--- Skipping Route Test (Model not found) ---")