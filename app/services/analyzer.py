...
def get_practical_advice(ingredient_results: List[IngredientDetails]) -> dict:
    """
    ...
    if not highest_group:
        ...
        max_conf = 0.0
        for grp, conf in group_conf_pairs:
            if grp and highest_group and (grp == highest_group or (("1" in grp and "1" in highest_group) or ("2" in grp and "2" in highest_group) or ("3" in grp and "3" in highest_group))):
                if conf > max_conf:
                    max_conf = conf
